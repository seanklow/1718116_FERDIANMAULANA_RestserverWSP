...
class Barang extends RESTController
{
  ...
  public function index_post()
  {
    $data = [
      ...
      'id_tipebarang' => $this->post('id_tipebarang', true),
      'nama_liquid' => $this->post('nama_liquid', true),
      'detail_barang' => $this->post('detail_barang', true),
      'nicotine' => $this->post('nicotine', true)
    ];
    $simpan = $this->brg->add($data);
    if ($simpan['status']) {
      $this->response([
        'status' => true,
        'msg' => $simpan['data'] . ' liquid di tambahkan'
  ...
  public function index_put()
  {
    $data = [
      ...
      'id_tipebarang' => $this->put('id_tipebarang', true),
      'nama_liquid' => $this->put('nama_liquid', true),
      'detail_barang' => $this->put('detail_barang', true),
      'nicotine' => $this->put('nicotine', true)
    ];
    ...
    if ($simpan['status']) {
      ...
      if ($status > 0) {
        $this->response([
          'status' => true,
          'msg' => $simpan['data'] . ' liquid Telah di ubah'
        ], RestController::HTTP_OK);
      } else {
        $this->response([
          'status' => false,
          'msg' => 'Tidak ada liquid yang di ubah'
        ], RestController::HTTP_BAD_REQUEST);
      }
    } else {
      $this->response([
